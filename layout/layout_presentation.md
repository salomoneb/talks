...
* As web developers, we make things **for the web**. It's essential that we know how to format content in a presentable way.
...
#### :bulb: [DEMO 4](https://codepen.io/soluhmin/pen/KGNgRe)
...
### Floats
* `float` property accepts `none`, `left`, or `right` values.
...
### Positioning
The `position` CSS property specifies how an element is positioned in a document. Positioned elements (elements with non-static positioning) can be placed along the page coordinates using `top`, `right`, `bottom`, and `left` properties.

We’ll review five types of element positioning: `static`, `relative`, `absolute`, `fixed`, and `sticky`.


#### Static
* The default positioning for all elements in the normal flow.
* `top`, `bottom`, `right`, and `left` properties have no effect.

#### Relative
* The element is positioned according to the normal flow, but `top`, `bottom`, `right`, and `left` properties can offset the element’s positioning.
* This offset does not affect the position of other elements.
* Relatively positioned items create a new stacking context when the value of `z-index` is not `auto`. This means that they appear over other elements that don’t have z-indexes.

#### Absolute
* The element is removed from the normal document flow and positioned relative to its closest positioned ancestor.
* Its final position is determined by `top`, `bottom`, `right`, and `left` properties.
Margin collapse isn’t an issue with absolutely positioned elements.

#### Fixed
* The element is removed from the normal flow and positioned relative to the viewport, which acts as its containing block.
* However, if an ancestor element has a `transform`, `perspective`, or `filter` property, the fixed element is positioned relative to that.

#### Sticky
* The element “sticks” to part of the viewport as the user scrolls.
* Requires a coordinate value.
* Does not work if the parent element has an `overflow` property with the values `hidden`, `scroll`, or `auto`.
* **Does not have great support.** Only has full support on the latest versions of Firefox and Safari. Has partial support on Chrome and no support on IE or Edge 15 and below.

#### Takeaways
Positioned elements are best for individual placement, not constructing entire page flows.

## Wrapping up
* We’ve seen how content, border, margin, and padding work together to form boxes that comprise web page layout.
